...
from model_am import Transducer
...
from icefall.utils_ctc import AttributeDict, MetricsTracker, setup_logger, str2bool, encode_supervisions
...
from icefall.graph_compiler_phone import CtcTrainingGraphCompiler

##  for alignment 
from icefall.ali_CE import (
...
def get_parser():
    ...
    parser.add_argument(
        "--exp-dir",
        type=str,
        default="pruned_transducer_stateless4/exp_musan_addCTC_addCE_BIES",
    ...
    params = AttributeDict(
        {
            ...
            "ali_dir" : "/workspace/icefall/egs/radiology_v2/data/ali_BIES",
            "alpha" : 0.5,
            "delta" : 0.1,
        }
    )
...
def compute_loss(
    ...
    CE = torch.nn.CrossEntropyLoss(label_smoothing=0.1, ignore_index=0, reduction='sum')
    
    with torch.set_grad_enabled(is_training):
        ctc_output, simple_loss, pruned_loss = model(
            x=feature,
            x_lens=feature_lens,
            y=y,
        ...
        supervision_segments, token_ids = encode_supervisions(
            supervisions, subsampling_factor=params.subsampling_factor, token_ids=token_ids,
        )
        ...
        mask = lookup_alignments(
            cut_ids=cut_ids,
            alignments=ali,
            num_classes=ctc_output.shape[2],
        ).to(ctc_output)
        min_len = min(ctc_output.shape[1], mask.shape[1])
        CE_loss = CE(ctc_output[:, :min_len, :].view(-1,params.vocab_size), mask[:, :min_len].contiguous().view(-1).to(torch.int64))
        assert CE_loss.requires_grad == is_training
        loss += params.delta * CE_loss
...
def run(rank, world_size, args):
    ...
    train_ali_filename = Path(params.ali_dir) / "train.pt"
    subsampling_factor, train_ali = load_alignments(train_ali_filename)
